...
/**
 * Partials begin with a greater than sign, like {{> box}}. Partials are rendered at runtime (as
 * opposed to compile time), so recursive partials are possible. Just avoid infinite loops. They
 * also inherit the calling context.
 *
 * @author edgar.espina
 * @since 0.1.0
 */
class Partial extends HelperResolver {
  ...
  private static final List<List<PathExpression>> OVERRIDE_PROPERTIES =
      Arrays.asList(PathCompiler.compile("size"), PathCompiler.compile("empty"));
  ...
  protected void merge(final Context context, final Writer writer) throws IOException {
    try {
      ...
      if (this.partial != null) {
        ...
        inlineTemplates.put(
            "@partial-block",
            new PartialBlockForwardingTemplate(
                this, this.partial, inlineTemplates.get("@partial-block"), callee, handlebars));
      }
        ...
        try {
          ...
          if (exists(invocationStack, source.filename())) {
            ...
            if (invocationStack.isEmpty()) {
              reason =
                  String.format(
                      "infinite loop detected, partial '%s' is calling itself", source.filename());
            ...
            HandlebarsError error =
                new HandlebarsError(caller.filename(), line, column, reason, text(), message);
      ...
      String reason =
          String.format(
              "The partial '%s' at '%s' could not be found",
  ...
  /**
   * Check for property conflicts and resolve them. Basically we ignore internal properties from Map
   * if they already present in current context.
   *
   ...
   */
  private static boolean exists(final List<TemplateSource> invocationStack, final String filename) {
